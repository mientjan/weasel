...
class Scroller
{
	...
	protected __zoomComplete:Function;
	protected __interruptedAnimation:boolean;
	protected __initialTouchLeft;
	protected __initialTouchTop;
	protected __zoomLevelStart;
	protected __lastScale;
	protected __enableScrollX;
	protected __enableScrollY;
	...
	constructor(callback:Function, options:IScrollerOptions = {})
	{
	...
	public doTouchStart(touches, timeStamp)
	{
		...
		{
			Animate.stop(self.__isAnimatingId);
	...
	public __publish(left:number, top:number, zoom:number, animate:boolean = false)
	{
		...
		if(animate && self.options.animating)
		{
			...
			self.__isAnimatingId = Animate.start(
				step, verify, completed,
				1000, wasAnimating ? easeOutCubic : easeInOutCubic
			);
			self.__isAnimating = !!self.__isAnimatingId;
		}
		else
